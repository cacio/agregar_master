	...
	if(isset($_REQUEST['act']) and !empty($_REQUEST['act'])){
		...
		case 'gravadados':
			...
			if($numnotasen1 > 0){
				...
				foreach ($lines as $line_num => $line) {
					...
					$notasa1txt->setNumeroNota(trim($numero_notasa1));
					$notasa1txt->setDataEmissao(date('Y-m-d',strtotime($dtemissao)));
					$notasa1txt->setCnpjCpf(trim($cnpj_cpf));
					$notasa1txt->setCodigoProduto(trim($cod_produto));
					$notasa1txt->setQtdPecas(trim(str_replace(',', '.', $qtdpecas)));
					$notasa1txt->setPeso(trim($peso));
					$notasa1txt->setPrecoUnitario(trim($precounitario));
					$notasa1txt->setEntSai(trim($entsai));
					$notasa1txt->setNumeroItemNota(trim($numitem));		
					$notasa1txt->setInscEstadual(trim($insc_est));
					$notasa1txt->setCfop(trim($cfop));
					$notasa1txt->setAliquotaIcms(trim($aliguotaicms));
					$notasa1txt->setCnpjEmp($_SESSION['cnpj']);
					$notasa1txt->setProdQtd(trim($peso));
				...
				if($numnotasen1cab > 0){
					...
					if(trim($peso_carcasa) < 001 and $tipo_r_v_det == 'R'){
						...
						array_push($xerro_notasen1, array(
							'codigo'=>''.$numero_nota_det.'',
							'cProd'=>''.trim($codigo_produto_det).'',
							'msg' => 'Linha #<b>{'.($line_num + 1).'}</b> - PESO DAS CARCACAS-> '.number_format($peso_carcasa,3,',','.').' !',					
						));
		...
		case 'testeent':
				...
				if(!empty($_REQUEST['cnpj'])){
					...
					echo "informe um cnpj de uma empresa para teste";
				}
		break;
		case 'testes':
