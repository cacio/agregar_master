<?php
	...
	if(isset($_REQUEST['act']) and !empty($_REQUEST['act'])){
		...
		case 'detalhenota':
			...
			if($tipo == 'E'){
				...
				for($i = 0; $i < $numen1; $i++){
					...
					if($tipo_r_v == 'R'){
						$subtotal		  = ($peso_carcasa * $preco_quilo);	
						$qtd_prod		  = $peso_carcasa;
					}else{
				...
				if(empty($erro)){
					...
					while ($arquivo = readdir($pasta)){
						...
						if ($arquivo != "." && $arquivo != ".."){
							...
							if($xml 	 =  simplexml_load_file($destino.$arquivo)){
			...
			if($numnotasenttxt > 0){		
				for($i = 0; $i < $numnotasenttxt;$i++){
					...
					foreach($xml->NFe->infNFe->det as $prod){
						...
						$daonotasen1 = new NotasEn1TxtDAO();
						$vetnotasen1 = $daonotasen1->VerificaCabecasPreenchidas($xml->NFe->infNFe->ide->nNF,trim($prod->prod->cProd),$_SESSION['cnpj'],($ii+1),$_SESSION['apura']['mesano']);
						$numnotasen1 = count($vetnotasen1);
						...
						if($xml->NFe->infNFe->ide->tpNF == 0){
							...
							if($numnotasenttxt == 0){
								...
								foreach($xml->NFe->infNFe->det as $prod){
									...
									//$prod->infAdProd
									$daonotasen1 = new NotasEn1TxtDAO();
									$vetnotasen1 = $daonotasen1->VerificaCabecasPreenchidas($xml->NFe->infNFe->ide->nNF,trim($prod->prod->cProd),$_SESSION['cnpj'],($i+1),$_SESSION['apura']['mesano']);
									$numnotasen1 = count($vetnotasen1);
			...
			if($numnotasaitxt > 0){
				...
				for($x = 0; $x < $numnotasaitxt; $x++){
						...
						foreach($xml2->NFe->infNFe->det as $prod2){
							...
							$daonotassa1    = new  NotasSa1Txt2DAO();//$numero,$nitem,$mesano,$cnpj
							$vetnotassa1    = $daonotassa1->pegaItemNota(trim($xml2->NFe->infNFe->ide->nNF),($y+1),$_SESSION['apura']['mesano'],$_SESSION['cnpj']);
							$codigo_produto = $vetnotassa1[0]['codigo_produto'];
							...
							if($numrl == 0){
								if(!empty($codigo_produto)){
									...
									array_push($err_produtoter,array(
										'codigo'=>''.trim($prod2->prod->cProd).'',
										'cnota'=>''.trim($xml2->NFe->infNFe->ide->nNF).'',
										'msg'=>' Relacione o produto ('.trim($prod2->prod->cProd).' '.$prod2->prod->xProd.') de terceiros, com o código do seu cadastro. ',
										"dados"=> array(
											'nNF'=>''.trim($xml2->NFe->infNFe->ide->nNF).'',
											'numero_item_nota'=>''.($y+1).'',
											'entsai'=>'Saida',
							...
							if($numrl > 0){
								...
								if($tipo == 'E'){
										...
										'cnota'=>''.trim($xml2->NFe->infNFe->ide->nNF).'',
										'msg'=>' Código do produto ('.trim($codigo_produto).' '.$prod2->prod->xProd.') ::  Produto relacionado com código do AGREGAR de entradas, em documento de saída. Verifique o relacionamento!::',
										"dados"=> array(
												'nNF'=>''.$xml2->NFe->infNFe->ide->nNF.'',
												'entsai'=>'Saida',
												'cliente'=>''.$xml2->NFe->infNFe->dest->xNome.'',
							...
							//alert_valoricms
							$imposto    = json_encode($prod2->imposto->ICMS);
							...
							if ($vicms <= 0){
								array_push($alert_valoricms,array(
									'codigo'=>''.trim($prod2->prod->cProd).'',
									'cnota'=>''.trim($xml2->NFe->infNFe->ide->nNF).'',
									'msg'=>' Código do produto ('.trim($prod2->prod->cProd).' '.$prod2->prod->xProd.') :: Produto não possui débito de ICMS ::',
									"dados"=> array(
										'nNF'=>''.$xml2->NFe->infNFe->ide->nNF.'',
										'entsai'=>'Saida',
										'cliente'=>''.$xml2->NFe->infNFe->dest->xNome.'',
		...
		case 'inserirnotamanual':
		    ...
		    $valoricms       = !empty($_REQUEST['valoricms']) ? str_replace(',', '.', str_replace('.', '', $_REQUEST['valoricms'])) : 0.00;
			$valoricmssubs   = !empty($_REQUEST['valoricmssubs']) ? str_replace(',', '.', str_replace('.', '', $_REQUEST['valoricmssubs'])) : 0.00;
					...
					if($topo[0] == 'NUMERONOTA' and $topo[1] == 'SERIE' and $topo[2] == 'CODIGOPRODUTO' and $topo[3] == 'ITEM' and $topo[4] == 'NCABECAS'){
						...
						for($i = 0; $i < count($rows); $i++){
							...
							if (is_numeric($NCABECAS)) {
								$daonotasen1 = new NotasEn1TxtDAO();
								$vetnotasen1 = $daonotasen1->VerificaIdCabecas2($NUMERONOTA,$CODIGOPRODUTO,$_SESSION['cnpj'],$ITEM,$_SESSION['apura']['mesano']);	
								$numnotasen1 = count($vetnotasen1);
						...
						if(trim($xml->NFe->infNFe->det->prod->CFOP) < 5000 and $vetcfop[0]['devolucao'] == 'N'){
							...
							if($numnotasaitxt == 0 or $itemserie == 0){
								...
								foreach($xml->NFe->infNFe->det as $prod){
									...
									$vettrc =  $daotrc->VerificaRelacionamento($indForn,trim($prod->prod->cProd),$_SESSION['cnpj']);
									if(count($vettrc) > 0){
									...
									$notasa1txt->setDataEmissao(date('Y-m-d',strtotime($dEmi[0])));
									$notasa1txt->setCnpjCpf($cnpj_cpf2);
									$notasa1txt->setCodigoProduto(trim($cprod));
									$notasa1txt->setQtdPecas(trim(0));
									$notasa1txt->setPeso(trim($prod->prod->qCom));
									$notasa1txt->setPrecoUnitario(trim($prod->prod->vUnCom));
									$notasa1txt->setEntSai($entsai_sai);
