			...
			if($numresm > 0){
				...
				if($installConfig->apuracao == '2'){
					...
					if($numprodutor > 0){
						...
						$vetnotassa1 = $daonotassa1->PegaValorDevolucaoSidaparaEntrada($where7);
						$numnotassa1 = count($vetnotassa1); 
						
						if($numnotassa1 > 0){
							$notasa1  = $vetnotassa1[0];
							$valordev = $notasa1->getValorDevolucao();
						}else{
							$valordev = 0;
						}

						$base       = (($vivo - $valor_glos) - $valordev);
