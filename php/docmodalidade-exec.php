	...
	if(isset($_REQUEST['act']) and !empty($_REQUEST['act'])){
		...
		switch($act){
			...
			case 'inserir':
				...
				$vet = $dao->ListaDocumentosModalidade($idmd);
				$num = count($vet);
				...
				foreach($ids as $key=>$value){
					...
					$mds = new ModalidadeDocumento();	
		
					$mds->setIdModalidade($idmd);
					$mds->setIdDocumento($value);
					if($value){				
						$dao->inserir($mds);	
					}
				}	
			...
			case 'listadocumento':	
				...
				$idmodenv = $_REQUEST['idmodenv'];
				
				$dao = new DocumentosDAO();
				$vet = $dao->ListaDocumentosPorModalidadeEnvio($idmodenv);
				$num = count($vet);
				$results = array();
