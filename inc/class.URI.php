...
class URI
{
	...
	protected function ResquestUri(){
		
		$urlphp 	   = explode('/php',$_SERVER['REQUEST_URI'])[1];
		$parturl       = explode('?', explode('/',$urlphp)[1])[0];
		
		if(empty($parturl)){
			$parturl = explode('/',$urlphp)[1];
		}else{
			$parturl = explode('?', explode('/',$urlphp)[1])[0];
		}
		
		self::$request = $parturl;
		
		return trim(self::$request);
	}
	...
	public function NomeUrl(){
		self::$nameurl = self::ResquestUri();
		return self::$nameurl;
	}
	/**
